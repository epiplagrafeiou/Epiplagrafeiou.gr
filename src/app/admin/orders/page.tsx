...
interface Order {
    ...
    companyDetails?: { name: string; vat: string; taxOffice: string; address: string; };
    items: OrderItem[];
    total: number;
    status: 'Pending' | 'Shipped' | 'Delivered' | 'Cancelled' | 'Pending Payment';
    createdAt: Timestamp;
}
                                    ...
                                    <TableRow key={order.id}>
                                        ...
                                        <TableCell>
                                            <div className="font-medium flex items-center gap-2">
                                                {order.companyDetails ? <Building className="h-4 w-4 text-muted-foreground" /> : <User className="h-4 w-4 text-muted-foreground" />}
                                                {order.companyDetails?.name || order.customerDetails.name}
                                            </div>
                                            <div className="text-sm text-muted-foreground">{order.customerDetails.email}</div>
                                            {order.companyDetails && (
                                                <div className="text-xs text-muted-foreground mt-1">
                                                    ΑΦΜ: {order.companyDetails.vat}
                                                </div>
                                            )}
                                        </TableCell>
                                        <TableCell>
