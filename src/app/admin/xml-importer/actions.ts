...
export async function syncProductsFromXml(url: string): Promise<Product[]> {
  try {
    ...
    if (!productArray || !Array.isArray(productArray)) {
      ...
      throw new Error(
        'The XML feed does not have the expected structure. Could not find a product array at `megapap.products.product`.'
      );
    }
    ...
    if (error instanceof Error) {
      throw new Error(
        `Could not parse XML from the provided URL. Please check the URL and XML format. Details: ${error.message}`
      );
    }
